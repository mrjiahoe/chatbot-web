...
const tableConfigSchema = z.object({
    ...
    table_kind: z.enum(['entity', 'relationship', 'lookup', 'summary', 'sensitive', 'system', 'unknown']).optional(),
...
const PLANNER_HIDDEN_TABLES = new Set([
    'base_account',
...
function normalizeColumnNameForClassification(column) {
    if (!column) {
        return '';
    }
    ...
    return normalizeIdentifier(column.name || column.normalizedName || '');
}

export function inferTableKind({ tableName, source, columns = [] }) {
    const normalizedTableName = normalizeIdentifier(tableName || source || '');
    const normalizedSource = normalizeIdentifier(source || tableName || '');
    const columnNames = columns
        .map(normalizeColumnNameForClassification)
        .filter(Boolean);
    const idColumnCount = columnNames.filter((name) => name.endsWith('_id')).length;
    const totalPrefixedColumnCount = columnNames.filter((name) => name.startsWith('total_')).length;
    const hasMetricPair = columnNames.includes('metric_name') && columnNames.includes('metric_value');

    if (
        normalizedTableName.startsWith('chatbot_schema_') ||
        normalizedSource.startsWith('chatbot_schema_') ||
        normalizedTableName.startsWith('auth_')
    ) {
        return 'system';
    }

    if (
        PLANNER_HIDDEN_TABLES.has(normalizedTableName) ||
        normalizedTableName.includes('account') ||
        normalizedSource.includes('account')
    ) {
        return 'sensitive';
    }

    if (
        normalizedTableName.includes('totalstats') ||
        normalizedTableName.includes('summary') ||
        normalizedTableName.includes('metric') ||
        normalizedTableName.includes('dashboard') ||
        hasMetricPair ||
        totalPrefixedColumnCount >= 3
    ) {
        return 'summary';
    }

    if (
        normalizedTableName.includes('level') ||
        normalizedTableName.includes('type') ||
        normalizedTableName.includes('category') ||
        normalizedTableName.includes('sector') ||
        normalizedTableName.includes('cluster')
    ) {
        return 'lookup';
    }

    if (idColumnCount >= 2 && columnNames.length <= 12) {
        return 'relationship';
    }
...
export function inferPlannerEnabled({ tableName, tableKind }) {
    const normalizedTableName = normalizeIdentifier(tableName || '');
    const resolvedTableKind = tableKind || inferTableKind({ tableName });

    if (PLANNER_HIDDEN_TABLES.has(normalizedTableName)) {
        return false;
    }

    return !['sensitive', 'system'].includes(resolvedTableKind);
}

export function resolveTablePlanningMetadata({
    tableName,
    source,
    columns = [],
    tableKind = null,
    ...
    const resolvedTableKind = tableKind || inferTableKind({
        tableName,
        source,
        columns,
    });
    ...
    return {
        tableKind: resolvedTableKind,
        plannerEnabled:
            plannerEnabled === null || plannerEnabled === undefined
                ? inferPlannerEnabled({ tableName, tableKind: resolvedTableKind })
                : Boolean(plannerEnabled),
    };
}

function getPlannerHint(tableKind) {
    switch (tableKind) {
        case 'entity':
            return 'Preferred for row-level records, filtered counts, and direct breakdowns.';
        case 'relationship':
            return 'Use when connecting entity tables across approved joins.';
        case 'lookup':
            return 'Useful for labels, categories, and small reference dimensions.';
        case 'summary':
            return 'Use for precomputed totals or metrics, not as the first choice for filtered person-level questions.';
        case 'sensitive':
            return 'Sensitive table. Keep out of planner context unless explicitly re-enabled.';
        case 'system':
            return 'System metadata table. Not intended for analytics planning.';
        default:
            return 'General-purpose analytics table.';
    }
}

function comparePlannerTables(left, right) {
    const leftPriority = TABLE_KIND_PRIORITY[left.tableKind] ?? TABLE_KIND_PRIORITY.unknown;
    const rightPriority = TABLE_KIND_PRIORITY[right.tableKind] ?? TABLE_KIND_PRIORITY.unknown;
...
function buildTableEntry(table) {
    ...
    const planningMetadata = resolveTablePlanningMetadata({
        ...
        plannerEnabled: table.planner_enabled,
    });
    ...
    return {
        ...
        columns,
        joins,
        plannerEnabled: planningMetadata.plannerEnabled,
        tableKind: planningMetadata.tableKind,
    };
}
...
async function parseSchemaFromSupabase(supabase) {
    ...
    try {
        ...
        let response = await supabase
            ...
            .select('table_name, provider, source, column_name, column_type, enabled, allow_in_planner, table_kind')
            .eq('enabled', true)
        ...
        if (error && isMissingPlannerMetadataColumnError(error)) {
        ...
        try {
            ...
            if (!joinsError) {
                ...
                console.warn('Falling back from Supabase schema joins registry:', joinsError.message);
            }
        } catch {
            joinRows = [];
        }
        ...
        data.forEach((row) => {
            ...
            if (!groupedTables.has(key)) {
                groupedTables.set(key, {
                    ...
                    source: row.source,
                    planner_enabled: row.allow_in_planner,
...
export async function buildSchemaRegistry({ dataContext, supabase, registrySupabase = null, skipCache = false }) {
    // Check cache first
    if (!skipCache) {
    ...
    const metadataSupabase = registrySupabase || supabase;
    ...
    const databaseSchema = await parseSchemaFromSupabase(metadataSupabase);
...
export function getSchemaPrompt(registry) {
    return JSON.stringify(
        Array.from(registry.values())
            .filter((table) => table.plannerEnabled !== false)
            .sort(comparePlannerTables)
            .map((table) => ({
                table: table.name,
                display_name: table.displayName,
                provider: table.provider,
                table_kind: table.tableKind || 'unknown',
                planning_hint: getPlannerHint(table.tableKind),
