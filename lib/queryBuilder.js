...
const analysisSchema = z.object({
    ...
    analysis: z.enum(['trend', 'comparison', 'distribution']),
    table: z.string().min(1),
    column: z.string().min(1),
...
export function validateStructuredRequest(payload, registry) {
    ...
    if (parsed.type === 'query') {
        ...
        if (parsed.operation === 'select' && groupBy.length > 0) {
            throw new Error('group_by is only supported for count, average, and sum operations.');
        }
    ...
    if ((parsed.analysis === 'trend' || parsed.analysis === 'comparison') && !groupBy) {
...
function compileSqlColumn(columnRef, tableAliases) {
    ...
    return `${tableAliases.get(columnRef.table)}.\`${columnRef.column}\``;
}
...
function aggregateRows(rows, request) {
    ...
    if (request.operation === 'select') {
        ...
        const limitedRows = sortedRows.slice(0, request.limit || sortedRows.length);
...
export async function executeSupabaseQuery(supabase, plan) {
    ...
    request.columns.forEach((column) => {
        if (column.kind === 'column') {
            selectColumns.set(column.column, column.column);
        }
    });
    ...
    request.order_by.forEach((order) => {
        if (order.column.kind === 'column') {
            selectColumns.set(order.column.column, order.column.column);
        }
    });
    ...
    const addFilterColumns = (filterGroup) => {
        ...
        filterGroup.conditions.forEach((condition) => {
            ...
            if (condition.column.kind === 'column') {
                selectColumns.set(condition.column.column, condition.column.column);
            }
        });
    };
    ...
    const { data, error } = await supabase
        .from(request.source)
        .select(Array.from(selectColumns.values()).join(', '));
