...
export function validateStructuredRequest(payload, registry) {
    ...
    const metricColumn = parsed.column
        ? assertColumn(tableConfig, parsed.column).normalizedName
        : null;
    ...
    if (
        (parsed.analysis === 'trend' ||
            ...
            parsed.analysis === 'period_change') &&
        !groupBy
    ) {
        throw new Error(`${parsed.analysis} analysis requires a group_by column.`);
    }

    if (parsed.analysis !== 'data_quality' && !metricColumn) {
        throw new Error(`${parsed.analysis} analysis requires a column.`);
    }
...
function areCaseInsensitiveStringEqual(leftValue, rightValue) {
    return (
        typeof leftValue === 'string' &&
        typeof rightValue === 'string' &&
        normalizeComparableString(leftValue) === normalizeComparableString(rightValue)
    );
}
...
function matchesCondition(row, condition) {
    ...
    if (condition.operator === '=') {
        return areCaseInsensitiveStringEqual(leftValue, condition.value)
            ? true
    ...
    if (condition.operator === 'in') {
        return (
            ...
            condition.value.some((candidateValue) =>
                areCaseInsensitiveStringEqual(leftValue, candidateValue)
                    ? true
                    : candidateValue === leftValue
            )
        );
    }
...
function collectRequestColumnRefs(request) {
    const refs = new Map();

    const addColumn = (columnRef) => {
        if (!columnRef || columnRef.kind !== 'column') {
            return;
        }

        refs.set(`${columnRef.table}:${columnRef.outputLabel}`, columnRef);
    };

    request.columns.forEach(addColumn);
    request.group_by.forEach(addColumn);

    request.order_by.forEach((order) => {
        addColumn(order.column);
    });
...
function buildJoinChildrenMap(request) {
    const children = new Map();

    request.joins.forEach((join) => {
        if (!children.has(join.fromTable)) {
            children.set(join.fromTable, []);
        }

        children.get(join.fromTable).push(join);
    });

    return children;
}

function buildSupabaseSelectClauseForTable({
    tableName,
    request,
    columnsByTable,
    joinChildren,
}) {
    const scalarColumns = Array.from(columnsByTable.get(tableName) || []);
    const relationSegments = (joinChildren.get(tableName) || []).map((join) => {
        const targetTable = request.joinedTables.get(join.table);
        const targetSource = targetTable?.source || join.table;
        const nestedSelect = buildSupabaseSelectClauseForTable({
        ...
        return `${targetSource}${join.type === 'left' ? '' : '!inner'}(${nestedSelect || '*'})`;
    });

    return [...scalarColumns, ...relationSegments].join(', ');
}

function buildSupabaseSelectClause(request) {
    const columnRefs = collectRequestColumnRefs(request);
    const columnsByTable = new Map();

    columnRefs.forEach((columnRef) => {
        if (!columnsByTable.has(columnRef.table)) {
            columnsByTable.set(columnRef.table, new Set());
        }

        columnsByTable.get(columnRef.table).add(columnRef.sourceColumn || columnRef.column);
    });

    const joinChildren = buildJoinChildrenMap(request);
    return buildSupabaseSelectClauseForTable({
...
function buildJoinPathByTable(request) {
    const paths = new Map([[request.table, []]]);
    const pending = [...request.joins];
    ...
    while (pending.length > 0) {
        ...
        for (let index = 0; index < pending.length; index += 1) {
            ...
            if (!paths.has(join.fromTable)) {
                continue;
            }

            const fromPath = paths.get(join.fromTable);
            const targetSource = request.joinedTables.get(join.table)?.source || join.table;
            paths.set(join.table, [...fromPath, targetSource]);
...
function readNestedRelationValue(row, path) {
    let current = row;
...
function flattenSupabaseRows(rows, request) {
    const columnRefs = collectRequestColumnRefs(request);
    const joinPaths = buildJoinPathByTable(request);

    return rows.map((row) => {
        const flattenedRow = {};

        columnRefs.forEach((columnRef) => {
            const relationValue = columnRef.table === request.table
                ? row
                : readNestedRelationValue(row, joinPaths.get(columnRef.table) || []);
            const value = relationValue == null
                ? null
                : relationValue[columnRef.sourceColumn || columnRef.column] ?? null;

            flattenedRow[columnRef.outputLabel] = value;
        });
...
export async function executeSupabaseQuery(supabase, plan) {
    ...
    const selectClause = request.joins.length > 0
        ? buildSupabaseSelectClause(request)
        : Array.from(
            collectRequestColumnRefs(request).reduce((columns, columnRef) => {
                if (columnRef.table === request.table) {
                    columns.add(columnRef.sourceColumn || columnRef.column);
                }
    ...
    const normalizedRows = request.joins.length > 0
        ? flattenSupabaseRows(data || [], request)
        : data || [];

    return aggregateRows(normalizedRows, request);
}
