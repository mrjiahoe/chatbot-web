...
export async function signUp({ email, password, metadata }) {
    ...
    if (!response.ok) {
        return {
            data: null,
            error: {
                message: payload?.error || 'Unable to sign up right now.',
            },
        };
    }
...
/**
 * Logs in an existing user using either their email or username plus password.
 */
export async function signIn({ identifier, password }) {
    ...
    if (!response.ok) {
        return {
            data: null,
            error: {
                message: payload?.error || 'Unable to sign in right now.',
            },
        };
    }
