...
export function getAssignableRoleOptions(actorRole) {
    ...
    if (canManageSuperAdminAssignments(actorRole)) {
        return ROLE_OPTIONS.filter((option) => option.managed);
    }

    return ROLE_OPTIONS.filter((option) => option.managed && option.value !== 'super_admin');
}
...
export function canAccessSchemaRegistry(role) {
...
export function canManageSuperAdminAssignments(role) {
    return normalizeRole(role) === 'super_admin';
}
