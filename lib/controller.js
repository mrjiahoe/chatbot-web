...
function isStudentProgrammeCountQuestion(message) {
    const normalizedMessage = String(message || '').toLowerCase();
    ...
    return (
        ...
        /\bhow many\b|\bnumber of\b|\bcount\b|\btotal\b/.test(normalizedMessage) &&
        /\b(?:year|yr|form)\s+(?:\d{1,2}|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)\b/.test(normalizedMessage) &&
        /\bstream\b|\bprogramme\b|\bprogram\b|\btrack\b|\bart\b|\bscience\b|\bcommerce\b/.test(normalizedMessage)
    );
}

function normalizeProgrammeValue(value) {
    const normalized = String(value || '')
        .trim()
        .replace(/\bstudents?\b.*$/i, '')
        .replace(/\bin\b.*$/i, '')
        .replace(/\bfor\b.*$/i, '')
        .replace(/\bclass\b.*$/i, '')
        .trim();
    ...
    if (lowercase === 'art stream') {
        return 'Art';
    }

    if (lowercase === 'science stream') {
        return 'Science';
    }

    if (lowercase === 'commerce stream') {
        return 'Commerce';
    }

    return normalized.replace(/\bstream\b$/i, '').trim() || normalized;
}

function extractProgrammeLabel(message) {
    const rawMessage = String(message || '').trim();
    const patterns = [
        /\bin\s+([a-z][a-z\s/&-]{1,40})\s+stream\b/i,
        /\b([a-z][a-z\s/&-]{1,40})\s+stream\b/i,
        /\bprogramme\s+([a-z][a-z\s/&-]{1,40})\b/i,
        /\bprogram(?:me)?\s+([a-z][a-z\s/&-]{1,40})\b/i,
    ];
    ...
    for (const pattern of patterns) {
        ...
        if (match?.[1]) {
            const normalized = normalizeProgrammeValue(match[1]);
    ...
    if (normalizedMessage.includes('art stream')) {
        return 'Art';
    }

    if (normalizedMessage.includes('science stream')) {
        return 'Science';
    }

    if (normalizedMessage.includes('commerce stream')) {
        return 'Commerce';
    }
...
function buildProgrammeFilterVariants(label) {
    const normalized = normalizeProgrammeValue(label);
    ...
    variants.add(titled);
    variants.add(`${titled} Stream`);
    variants.add(`${normalized} Stream`);
...
function buildStudentProgrammeYearCountRequest(message, registry) {
    if (!isStudentProgrammeCountQuestion(message)) {
        return null;
    }

    const schoolingProfileTable = registry.get('base_schoolingprofile');
    const yearLevelLabel = extractYearLevelLabel(message);
    const programmeLabel = extractProgrammeLabel(message);

    if (!schoolingProfileTable || !yearLevelLabel || !programmeLabel) {
        return null;
    }

    const yearFilterValues = buildYearLevelFilterVariants(yearLevelLabel);
    const programmeFilterValues = buildProgrammeFilterVariants(programmeLabel);

    if (!yearFilterValues.length || !programmeFilterValues.length) {
        return null;
    }
    ...
    if (getColumnConfig(schoolingProfileTable, 'schooling_year')) {
        yearConditions.push({
            column: 'schooling_year',
            operator: 'in',
            value: yearFilterValues,
        });
    }

    if (getColumnConfig(schoolingProfileTable, 'classroom_name')) {
        yearConditions.push({
    ...
    if (getColumnConfig(schoolingProfileTable, 'academic_programme')) {
        programmeConditions.push({
            column: 'academic_programme',
            operator: 'in',
            value: programmeFilterValues,
        });
    }

    if (getColumnConfig(schoolingProfileTable, 'academic_programme_other')) {
        programmeConditions.push({
            column: 'academic_programme_other',
            operator: 'in',
            value: programmeFilterValues,
        });
    }
    ...
    return {
        ...
        filters: {
            ...
            conditions: [
                ...
                programmeConditions.length === 1
                    ? programmeConditions[0]
                    : {
                        logic: 'or',
...
function buildStructuredRetryRequest({ message, registry, error }) {
    ...
    return (
        buildStudentProgrammeYearCountRequest(message, registry) ||
